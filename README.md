...
* To setup a game copy `app/user/config.yaml.example` to `app/usr/config.yaml` and change it.
...
* Each user needs to select their team when they first visit the website. It will be saved for 24h so users may need to select their team again for longer competitions.
