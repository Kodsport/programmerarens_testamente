def generateFile(flag: str):
    import base64
    return f'''indata är en lista bestående av siffror, skriv det minsta!
import base64
...
def kollaOmRätt(funktion):
    ...
    for i in range(10):
        ...
        with Capturing() as output:
            funktion(ls)
        if int(output[0]) != min(ls):
            print(f'Tyvärr! För listan {{ls}} svarade ni {{int(output[0])}}, men svaret var {{min(ls)}}')
            break
    else:
        print(f'Sal {{sal}}')
